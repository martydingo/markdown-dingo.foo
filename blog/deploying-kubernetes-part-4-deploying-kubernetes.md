---
title: Deploying Kubernetes with Cilium
series: 
...
The fourth article of the Deploying Kubernetes series. This article follows on the previous instructional, *Deploying Network Storage*, where we configured our network file server for later usage by workloads running on our Kubernetes stack.

## Virtual Machine Creation

### Control Node

1. Create a new control node virtual machine

   1. Click _Create/Register VM_
   2. Select _Create a new virtual machine_, press next
   3. Enter the following details:
      1. **Name:** I will be using _a.control-node.kubernetes.dingo.services_, just in case we want to add a second control node into the cluster for redundancy.
      1. **Compatibility:** The latest ESXI version _virtual machine_
      1. **Guest OS Familt:** _Linux_
      1. **Guest OS Version:** _Red Hat Enterprise Linux 9 (64-bit)_
   4. Select the datastore. e.g. `datastore1`
   5. Configure the VM
      1. **CPU:** The maximum the dropdown reflects.
      1. **Memory:** _16 GB_
         **NOTE:** This is very generous for a control node, I've safely ran control nodes on 8GB of RAM.
      1. **Hard Disks:**
         1. **First Hard Disk:** 64GB (This will be our OS, we'll be using LVM as so we can extend the size later on if required)
      1. **Network Adaptor**s:
         1. **First Network Adaptor:** _VM Network_
         1. **Second Network Adaptor:** _SAN Network_
      1. **CD/DVD Drive 1:** rhel-baseos-9.0-x86_64-dvd.iso

### Worker Nodes

1. Create three new worker node virtual machines
   **NOTE:** Two will also work, just undertake the same operations listed but for two virtual machines, not three.
   1. Click _Create/Register VM_
   2. Select _Create a new virtual machine_, press next
   3. Enter the following details:
      1. **Name:** I will be using \*[a,b,c].worker-node.kubernetes.dingo.services
      1. **Compatibility:** The latest ESXI version _virtual machine_
      1. **Guest OS Familt:** _Linux_
      1. **Guest OS Version:** _Red Hat Enterprise Linux 9 (64-bit)_
   4. Select the datastore. e.g. `datastore1`
   5. Configure the VM
      ...
      1. **Memory:** _32 GB_
      1. **Hard Disks:**
         1. **First Hard Disk:** 128GB (This will be our OS, and worker nodes hold images on disk, so we need to ensure some storage space for those)
      1. **Network Adaptor**s:
         1. **First Network Adaptor:** _VM Network_
         1. **Second Network Adaptor:** _SAN Network_
      1. **CD/DVD Drive 1:** rhel-baseos-9.0-x86_64-dvd.iso
...
An instructional will come at a later date, however the installation wizard is quite self-explanatory, barring a few details around hostname & domain configuration. Do note you can grab yourself a developer license for free from Red Hat's website by registering and following the steps to obtain a developer license. 

Do note that you can either make a static DHCP lease for each machine in our back-eend router following the initial boot of the machine, or you can statically set the IP addresses listed in our IP schema statically. In any case, from this point onwards, we'll assume that the systems are on the same IP address schema I am using.    

Once you have your system up and running Red Hat Enterprise Linux 9, and you can successfully SSH into each system, then proceed with the following steps. 

## System Preparation

1. **Reconfigure SELinux to 'Permissive' on all nodes**
...
3. **Disable `FirewallD` on all nodes**

```bash
systemctl disable --now firewalld
```

4. **Enable `br_netfilter` & `overlay` modules on all nodes**

   Configure the br_netfilter & overlay modules into the `modules-load.d/k8s.conf` configuration file
   
   The following commands will enable these modules at run time. 
   ```bash
   modprobe br_netfilter
   modprobe overlay
   ```
   
   The following commands will enable these modules at boot.
   ```bash
...
5. **Configure `net.ipv4.ip_forward` & `net.bridge.bridge-nf` kernel parameters on all nodes** 
...
6. Apply the previously written kernel parameters. 
...
Here lies a set of instructions to install *ContainerD*, which will be the container runtime of choice that we'll be using. 

1. Install `wget` if it isn't already installed, in my case I used a minimal installation of RHEL9 so I will need to install `wget`
   `dnf install -y wget`
2. Change your working directory to `/tmp`
   `cd /tmp`
1. Download the *ContainerD* runtime. Note that we're using version *1.6.8*,  but you should try and download the latest version available. 
   `wget https://github.com/containerd/containerd/releases/download/v1.6.8/containerd-1.6.8-linux-amd64.tar.gz`
2. Extract the downloaded runtime
   `tar xvf containerd-1.6.8-linux-amd64.tar.gz`
3. Apply *execute* permissions to the ContainerD binaries extracted
   `chmod +x bin/\*`
4. Move the binaries to a location that is located within `$PATH`
   `mv bin/\* /usr/local/sbin/`
5. Download the systemd service file
   `wget https://raw.githubusercontent.com/containerd/containerd/main/containerd.service`
6. Edit the downloaded containerd.service file, and change the configured path of containerd within the service file to `/usr/local/sbin`
	1. `vim containerd.service`
	2. Change the configured path of containerd within the service file to `/usr/local/sbin
	3. Save and close the `containerd.service` file
	   `:x`
7. Place the *ContainerD* service file into where it is required
   `mv containerd.service /usr/local/lib/systemd/system/containerd.service`
8. Refresh all systemd services, then enable/start *ContainerD*
    `systemctl daemon-reload && systemctl enable --now containerd`

9. Make a configuration directory for *ContainerD*, and save the default configuration to `/etc/containerd/config.toml`
   ```bash
   sudo mkdir -p /etc/containerd
   sudo containerd config default | sudo tee /etc/containerd/config.toml
   ```
   
12. Now reconfigure `ContainerD` by opening the configuration file with a text editor, and adjust the parameter listed to reflect what's listed below.  
	1. `vim /etc/containerd/config.toml`
	2. Set `SystemdCgroup` to `true` 
	```ini
	[plugins."io.containerd.grpc.v1.cri".containerd.runtimes.runc.options]
	    SystemdCgroup = true
	```
1. Restart *ContainerD* to enact the configuration changes 
...
wget https://github.com/opencontainers/runc/releases/download/v1.1.3/runc.amd64 -O runc
chmod +x runc
...
1. Install *kubelet*, *kubeadm* & *kubectl*, and then start the *kubelet* service

```bash
sudo dnf install -y kubelet kubeadm kubectl --disableexcludes=kubernetes
```

1.  Enable _kubelet_ service

```bash
	sudo systemctl enable --now kubelet
```

2. Appends hosts to `/etc/hosts`

```c
10.0.0.1        a.control-node.kubernetes.dingo.services
10.0.1.1        a.worker-node.kubernetes.dingo.services
10.0.2.1        b.worker-node.kubernetes.dingo.services
10.0.3.1        c.worker-node.kubernetes.dingo.services
```
...
nodeRegistration:
  kubeletExtraArgs:
    node-ip: 10.0.0.1,fd:0:0:0:be:0:0:1
  criSocket: unix:///var/run/containerd/containerd.sock
  imagePullPolicy: IfNotPresent
  name: a.control-node.kubernetes.dingo.services
  taints: null
---
apiServer:
  extraArgs:
    service-cluster-ip-range: 100.112.0.0/12,fd:0:0:0:fe:be::/108
...
controllerManager:
  extraArgs:
    ...
    cluster-cidr: 10.1.0.0/16,fd:0:0:0:befe::/80
    service-cluster-ip-range: 100.112.0.0/12,fd:0:0:0:fe:be::/108
    node-cidr-mask-size-ipv6: "96"
dns: {}
etcd:
  extraArgs:
    listen-metrics-urls: http://127.0.0.1:2381, http://10.0.0.1:2381
  local:
...
networking:
  dnsDomain: kubernetes.dingo.services
  podSubnet: 10.1.0.0/16,fd:0:0:0:befe::/80
  serviceSubnet: 100.112.0.0/12,fd:0:0:0:fe:be::/108
scheduler:
  extraArgs:
    bind-address: 0.0.0.0
---
...
```sh
kubeadm join 10.0.0.1:6443 --token yt8kfj.y71lcdn3vq0rwjvm \
	--discovery-token-ca-cert-hash sha256:1c8074e083a72d3ab7fbf2d358a8c0b006fd1e90fd5c94a54782742a979d7940
```

### Control Node only

For each worker node, label the nodes as workers via the control node.

`kubectl label node a.worker-node.kubernetes.dingo.services node-role.kubernetes.io/worker=worker`
`kubectl label node b.worker-node.kubernetes.dingo.services node-role.kubernetes.io/worker=worker`
`kubectl label node c.worker-node.kubernetes.dingo.services node-role.kubernetes.io/worker=worker`
...
2. Copy the following YAML code block into helm-cilium-values.yaml
...
k8sServiceHost: 10.1.0.1
k8sServicePort: 6443

cluster:
  # -- Name of the cluster. Only required for Cluster Mesh.
  name: kubernetes.dingo.services
...
ingressController:
  ...
  secretsNamespace:
    ...
    name: cilium-ingress-tls-secrets
...
hubble:
  ...
  peerService:
    ...
    clusterDomain: ckubernetes.dingo.services
  ...
  relay:
    ...
    image:
      ...
      tag:
        "v1.12.1"
        # hubble-relay-digest
      digest: "sha256:646582b22bf41ad29dd7739b12aae77455ee5757b9ee087f2d45d684afef5fa1"
      useDigest: true
      pullPolicy: "IfNotPresent"
  ...
  ui:
    ...
    backend:
      ...
      image:
        ...
        tag: "v0.9.1@sha256:c4b86e0d7a38d52c6ea3d9d7b17809e5212efd97494e8bd37c8466ddd68d42d0"
        pullPolicy: "IfNotPresent"
    ...
    frontend:
      ...
      image:
        ...
        tag: "v0.9.1@sha256:baff611b975cb12307a163c0e547e648da211384eabdafd327707ff2ec31cc24"
        pullPolicy: "IfNotPresent"
...
ipam:
  ...
  operator:
    ...
    clusterPoolIPv6PodCIDRList:
      - fd:0:0:0:be:fe::/112
      - fd:0:0:0:be:fe:1::/112
      - fd:0:0:0:be:fe:2::/112
      - fd:0:0:0:be:fe:3::/112
...
# the user must configure the routes to reach pods, either manually or by
# setting the auto-direct-node-routes flag.
ipv6NativeRoutingCIDR: "fd:0:0:0:be::/96"
...
etcd:
  ...
  clusterDomain: kubernetes.dingo.services
...
clustermesh:
  ...
  apiserver:
    ...
    image:
      ...
      digest: "sha256:c80a8d6ffdf7cab4699441496f628a09a31d0300e623cadb2837c86fa368c02f"
      useDigest: true
      pullPolicy: "IfNotPresent"
...
3. Run the following command to create the namespace for Cilium.
   `kubectl create ns cilium`
   
4. Finally, install Cilium using Helm.
   `helm install -n cilium cilium cilium/cilium -f helm-cilium-values.yaml`
